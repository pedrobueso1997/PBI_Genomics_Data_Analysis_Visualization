...
By plotting the loadings, we can see which cell lines are closer to others (we focuse primaly on the first PC, this is, the X axis). The complemented lines JKD, SCR and NUC are closer to the wild-type, which means that they were closer than others to a total complementation.
...
df = data.frame(PC1=loadings[,1], PC2=loadings[,2])
df = tibble::rownames_to_column(df, "cell_lines")
df$cell_lines = label_function(df$cell_lines, genomes)
...
\bibitem{1}
"RPubs - Análisis de Componentes Principales (Principal Component Analysis, PCA) y t-SNE", Online, Available: \url{https://rpubs.com/Joaquin_AR/287787}, Accessed: 28-Dec-2019.
\bibitem{2}
